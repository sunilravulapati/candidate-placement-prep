...
## Overview
Design a class to find the k-th largest element in a growing stream.

## Why Learn This?
Essential entry-level heap pattern used in technical screening interviews.

## Recognition Clues
Look for array/string traversal where heap properties or sliding boundaries reduce time complexity.

## Prerequisites
Basic heap syntax and fundamentals

## Concepts Used
Heap traversal, conditionals, pointers
...
### Approach
Traverse the input using standard heap techniques, updating state variables or hash maps as required.
...
## Pseudocode
```text
# Standard Heap solution pattern
initialize results
for item in input:
  process item
return result
