...
export default function ResultsScreen({ results, onClose }: ResultsScreenProps) {
  ...
  const questionCount = results?.questionCount ?? results?.questions?.length ?? 0;
  const accuracy = results?.accuracy ?? (questionCount > 0 ? Math.round((score / questionCount) * 100) : 0);
  ...
  const [bookmarks, setBookmarks] = useState<Record<string, boolean>>(() => {
    ...
    questions.forEach((q) => {
      if (q?.id) initial[q.id] = q.isBookmarked || false;
    });
    return initial;
  });
    ...
    <div className="space-y-6 max-w-4xl mx-auto pt-2">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <div className="relative z-10 space-y-3">
            ...
            <div>
              ...
              <h3 className="text-2xl font-black text-white mt-0.5">
                {score} <span className="text-xs text-slate-500 font-semibold">/ {questionCount}</span>
              </h3>
            </div>
          </div>
        </div>
        ...
        <div className="glass-card p-5 rounded-2xl border border-slate-800 relative overflow-hidden group">
          ...
          <div className="relative z-10 space-y-3">
            ...
            <div>
              ...
              <h3 className="text-2xl font-black text-white mt-0.5">{accuracy}%</h3>
            </div>
          </div>
        </div>
        ...
        <div className="glass-card p-5 rounded-2xl border border-slate-800 relative overflow-hidden group">
          ...
          <div className="relative z-10 space-y-3">
            ...
            <div>
              ...
              <h3 className="text-2xl font-black text-white mt-0.5">{formatTime(timeTaken)}</h3>
            </div>
          </div>
        </div>
        ...
        <div className="glass-card p-5 rounded-2xl border border-slate-800 relative overflow-hidden group">
          ...
          <div className="relative z-10 space-y-3">
            ...
            <div>
              <p className="text-slate-400 text-[10px] font-bold uppercase tracking-wider">Avg / Question</p>
              <h3 className="text-2xl font-black text-white mt-0.5">
      ...
      {questions.length === 0 ? (
        <div className="glass-card p-8 rounded-3xl border border-slate-800 text-center space-y-3">
          ...
          <p className="text-xs text-slate-400">Detailed question snapshots are not recorded for this session.</p>
        </div>
      ) : (
              ...
              <div className="glass-card p-6 rounded-3xl border border-slate-800 space-y-6">
                ...
                <div className="space-y-4">
                  <h4 className="text-base font-bold text-white leading-relaxed whitespace-pre-line">
                    {currentQuestion.description || currentQuestion.question || currentQuestion.title}
                  </h4>
                </div>
                ...
                <div className="space-y-3 pt-4 border-t border-slate-850">
                  ...
                  <div className="text-xs text-slate-400 leading-relaxed whitespace-pre-line bg-slate-950/40 p-4 rounded-2xl border border-slate-900/60 shadow-inner">
                    {currentQuestion.explanation || 'Option is the verified correct answer.'}
                  </div>
                </div>
              </div>
            )}
            ...
            <div className="glass-card p-5 rounded-3xl border border-slate-800 space-y-4">
              ...
              <div className="grid grid-cols-5 gap-2">
                {questions.map((q, idx) => {
                  ...
                  const qIsCorrect = q.isCorrect ?? (q.userAnswer === q.correctAnswer);
